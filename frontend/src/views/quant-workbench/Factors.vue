...
const search=computed({get:()=>route.query.q||'',set:q=>router.replace({query:{...route.query,q:q||undefined}})})
...
function restoreSelection(){const selected=sets.value.find(s=>String(s.id)===String(route.query.id));if(selected){form.value=clone(selected);editorOpen.value=true}}
async function load(){[catalog.value,sets.value,universes.value]=await Promise.all([quant.list('factor-catalog'),quant.list('factors'),quant.list('universes')]);restoreSelection()}
watch(()=>route.query.id,restoreSelection)
watch(editorOpen,open=>{if(!open&&route.query.id)router.replace({query:{...route.query,id:undefined}})})
function edit(row){form.value=clone(row);editorOpen.value=true;router.replace({query:{...route.query,id:row.id}})}
function save(){run(async()=>{const saved=form.value.id?await quant.update('factors',form.value.id,form.value):await quant.create('factors',form.value);sets.value=await quant.list('factors');edit(saved)})}
