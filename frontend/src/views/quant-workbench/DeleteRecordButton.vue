...
const props = defineProps({ resource: { type: String, required: true }, record: { type: Object, required: true }, disabled: Boolean })
...
<template>
  <Button type="button" size="sm" variant="outline" class="text-destructive hover:text-destructive" :disabled="disabled || busy" @click="open=true">删除</Button>
  <Dialog :open="open" @update:open="value=>{if(!busy)open=value}">
    <DialogContent>
      <DialogHeader>
        <DialogTitle>删除“{{record.name || record.id}}”？</DialogTitle>
