...
async function load(){[rows.value,backtests.value]=await Promise.all([quant.list('deployments'),quant.list('backtests')]);if(route.query.id){[detail.value,events.value]=await Promise.all([quant.get('deployments',route.query.id),quant.list(`deployments/${encodeURIComponent(route.query.id)}/events`)]);if(detail.value?.errorMessage){const message=`${detail.value.errorCode||'模拟组合失败'} · ${detail.value.errorMessage}`;if(message!==lastDetailError){lastDetailError=message;feedback.error(message,{duration:6000})}}else lastDetailError=''}else{detail.value=null;events.value=[];lastDetailError=''}}
...
<template>
 ...
 <div><dt class="muted">来源回测</dt><dd class="flex flex-wrap items-center gap-2"><QuantStatusBadge :status="sourceBacktest?.qualification?.status" /><span v-if="!sourceBacktest" class="muted">记录不可用</span><Button v-if="sourceBacktest" size="sm" variant="ghost" as-child><RouterLink :to="{path:'/quant/tasks',query:{type:'backtests',id:sourceBacktest.id}}">查看回测</RouterLink></Button></dd></div>
 ...
 </dl><div v-if="runtime.reasons?.length||Object.keys(runtime.corporateActionWarnings||{}).length" class="mt-3 border-t pt-3"><p v-for="reason in runtime.reasons" :key="reason" class="muted">{{label(reason)}}</p><p v-for="(dates,assetId) in runtime.corporateActionWarnings" :key="assetId" class="muted">资产 {{assetId}}：{{dates.join('、')}}</p></div></section><ResultReport :result="{...detail.state,...detail.result,qualification:null}" /><details class="quant-secondary"><summary>生命周期与账本事件</summary><DataTable :rows="events" /></details></template>
</template>
