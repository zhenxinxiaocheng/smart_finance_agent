...
import { createExperimentRefresh, explainExperimentError } from './experimentExplanation.js'
...
const silentRefresh = createExperimentRefresh(
  id => quant.experiments.get(id, { silent: true }),
  currentId,
...
async function load() {
  ...
  if (String(detail.value?.id || '') !== requestedId) detail.value = null
  loading.value = true
  loadError.value = null
  pollingPaused.value = false
  try {
    const value = await quant.experiments.get(requestedId)
    if (requestedId === currentId()) detail.value = value
  } catch (error) {
    if (requestedId === currentId()) loadError.value = explainExperimentError(error)
  } finally {
    if (requestedId === currentId()) loading.value = false
  }
}

async function poll() {
  try { await silentRefresh() } catch { pollingPaused.value = true }
}

const shouldPoll = computed(() => !pollingPaused.value && detail.value && ['QUEUED', 'RUNNING'].includes(detail.value.status))
onMounted(load)
watch(() => route.params.id, load)
usePoll(poll, () => shouldPoll.value)
</script>
...
<template>
  ...
  <template v-if="detail">
    ...
    <p v-if="pollingPaused" class="muted mb-3">自动刷新已暂停，可重新进入页面获取最新结果。</p>
