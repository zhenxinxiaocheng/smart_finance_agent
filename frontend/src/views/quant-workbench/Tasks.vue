...
async function load(){rows.value=await quant.list(type.value);detail.value=route.query.id?await quant.get(type.value,route.query.id):null;if(detail.value?.errorMessage){const message=`${detail.value.errorCode||'任务失败'} · ${detail.value.errorMessage}`;if(message!==lastDetailError){lastDetailError=message;feedback.error(message,{duration:6000})}}else lastDetailError=''}
...
const qualification=computed(()=>detail.value?.qualification?.status||(typeof detail.value?.qualification==='string'?detail.value.qualification:null)||detail.value?.result?.qualification?.status)
...
<template>
 ...
 <template v-if="detail"><QuantPageHeader :title="detail.name||'任务详情'"><template #back><Button variant="ghost" size="sm" @click="router.push({query:{...route.query,id:undefined}})">返回任务列表</Button></template><template #badges><QuantStatusBadge :status="detail.status" /><QuantStatusBadge :status="qualification" /></template><template #meta><span>{{formatTime(detail.createdAt)}}</span><span>{{label(detail.stage)}}</span><span v-if="detail.strategyVersionId" :title="detail.strategyVersionId">策略版本 · {{detail.strategyVersionId.slice(0,8)}}</span></template><template #actions><Button v-if="activeTask(detail.status)" variant="outline" :disabled="busy" @click="act('cancel')">取消任务</Button><Button v-if="['FAILED','CANCELLED'].includes(detail.status)" variant="outline" :disabled="busy" @click="act('retry')">重新运行</Button><Button v-if="type==='backtests'&&detail.status==='SUCCEEDED'&&qualification==='QUALIFIED'" as-child><RouterLink :to="{path:'/quant/deployments',query:{backtestId:detail.id}}">使用此版本创建模拟组合</RouterLink></Button><Button v-if="detail.strategyId" variant="outline" as-child><RouterLink :to="`/quant/strategies/${detail.strategyId}`">查看策略</RouterLink></Button><Button v-if="detail.strategyId" variant="outline" as-child><RouterLink :to="{path:`/quant/strategies/${detail.strategyId}`,query:{tab:type==='training-runs'?'training':'backtests'}}">配置并重新运行</RouterLink></Button><DeleteRecordButton :resource="type" :record="detail" :disabled="busy" @deleted="afterDelete" /></template></QuantPageHeader><div v-if="activeTask(detail.status)||detail.errorMessage" class="panel"><h3>任务状态</h3><p class="muted">当前阶段：{{label(detail.stage)}}</p><div v-if="activeTask(detail.status)&&typeof detail.progress==='number'" class="mt-3"><progress :value="detail.progress" max="100" aria-label="实际任务进度"></progress><p class="muted">{{detail.progress}}%</p></div><p v-else-if="activeTask(detail.status)" class="muted mt-3">当前阶段未报告百分比进度。取消将撤销结果发布，已开始的计算可能仍在后台结束。</p><p v-if="detail.errorMessage" class="error mt-3">{{detail.errorCode}} · {{detail.errorMessage}}</p><p v-if="qualification" class="mt-3">策略验证：{{label(qualification)}}</p></div><ResultReport hide-qualification-status :result="{...detail.result,qualification:detail.qualification}" /></template>
</template>
