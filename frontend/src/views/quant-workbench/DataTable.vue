...
const columns = computed(() => [...new Set(records.value.flatMap(row => row && typeof row === 'object' ? Object.keys(row) : ['value']))])
...
Object.assign(titles,{id:'记录编号',filledQuantity:'已成交数量',confirmedDate:'确认日期'})
const display = (col,value) => ['status','side','assetClass','strategyType','reason'].includes(col) ? label(value) : typeof value==='number' ? ['drawdown','weight','coverage','meanForwardReturn'].includes(col) ? `${(value*100).toFixed(2)}%` : value.toLocaleString(undefined,{maximumFractionDigits:6}) : format(value)
</script>
<template>
