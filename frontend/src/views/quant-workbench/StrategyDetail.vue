...
import { needsTraining, operationMessage } from './presentation.js'
...
const ml=computed(()=>needsTraining(form.value?.config?.strategyType))
...
async function loadEvaluationWindow(){
 ...
 }catch(error){if(sequence===evaluationSequence){evaluation.value=null;evaluationError.value=operationMessage(error);feedback.error(evaluationError.value,{duration:5000})}}
...
onMounted(()=>{if(tab.value==='versions'){historyOpen.value=true;tab.value='config'}loadLatest(load)});watch(()=>route.params.id,()=>loadLatest(load));watch(()=>[ml.value,tab.value,form.value],()=>{if(form.value&&!ml.value&&tab.value==='training')tab.value='backtests'});watch(()=>[tab.value,form.value?.universeId,range.value.startDate,range.value.endDate],()=>loadEvaluationWindow())
</script>
<template>
 <p v-if="error" class="error" role="alert">{{error}}</p>
 <template v-if="form">
 <QuantPageHeader :title="isNew?'创建策略':form.name">
 <template #back><QuantBackButton /></template>
 ...
 </dl><p class="muted mt-5 leading-relaxed">{{templateSummary}}</p></aside>
 </fieldset></form>

 <template v-else-if="(tab==='training'&&ml)||tab==='backtests'"><form class="panel" @submit.prevent="start(tab==='training'?'training-runs':'backtests')"><h3>{{tab==='training'?'启动训练':'启动回测'}}</h3><p v-if="ml&&tab==='backtests'" class="muted mb-3">回测开始日期须晚于模型最终留出截止日。</p><div v-if="tab==='backtests'" class="quant-backtest-period"><p v-if="evaluationLoading" class="muted mt-2">正在计算共同有效日期…</p><p v-else-if="evaluationError" class="error mt-2">{{evaluationError}}</p><template v-else-if="evaluation"><div class="toolbar mt-3"><Button v-for="suggestion in evaluation.suggestions" :key="suggestion.days" type="button" size="sm" variant="outline" :disabled="!suggestion.ready||busy" @click="applyEvaluationWindow(suggestion)">近 {{suggestion.days}} 个交易日{{suggestion.ready?'':`（仅有 ${suggestion.availableEvaluationDays} 日`+'）'}}</Button></div><p class="muted">最新共同可用日期：{{evaluation.latestAvailableDate||'暂无'}}；当前最多 {{evaluation.availableEvaluationDays}} 个有效交易日。</p><p v-if="hasSelectedRange" :class="insufficientEvaluation?'error mt-2':'muted mt-2'">当前所选区间：{{evaluation.selectedEvaluationDays||0}} 个共同有效交易日{{insufficientEvaluation?`，不足 ${evaluation.minimumEvaluationDays} 日，不能启动回测。`:'，可以启动回测。'}}</p></template></div><div class="fields"><label class="field">开始日期<Input v-model="range.startDate" type="date" required :max="range.endDate||undefined" /></label><label class="field">结束日期<Input v-model="range.endDate" type="date" required :min="range.startDate||undefined" /></label><label v-if="ml&&tab==='backtests'" class="field">训练产物<select v-model="range.modelTaskId" required><option value="" disabled>选择已完成训练</option><option v-for="t in trained" :key="t.id" :value="t.id">{{t.name||'训练记录'}} · 最终留出截止 {{t.result?.metrics?.evaluatedThrough||t.result?.metrics?.holdoutEnd||"见训练报告"}}</option></select></label></div><p v-if="form.status==='ARCHIVED'" class="muted mt-3">策略已归档，不能启动新任务。</p><p v-if="ml&&tab==='backtests'&&!trained.length" class="muted mt-3">暂无已完成训练，请先到训练页启动训练。</p><Button class="mt-4" type="submit" :disabled="busy||evaluationLoading||!!evaluationError||insufficientEvaluation||form.status==='ARCHIVED'||(tab==='training'&&!ml)">{{tab==='training'?'启动训练':'启动回测'}}</Button></form><div class="panel"><h3>任务记录</h3><div v-if="tab==='backtests'" class="toolbar"><label class="field">回测来源版本<select v-model="versionSelection" aria-label="筛选策略版本"><option value="current">当前版本（v{{form.revision}}）</option><option value="all">全部版本</option><option v-for="v in versions" :key="v.id" :value="v.id">v{{v.version}} · {{formatTime(v.createdAt)}}</option></select></label></div><p v-if="tab==='backtests'&&selectedVersion" class="muted mb-4">正在查看 v{{selectedVersion.version}} 的回测记录；启动回测使用当前保存的配置。</p><p v-if="!displayedTasks.length" class="muted mb-3">{{tab==='backtests'&&versionSelection==='current'?'当前版本暂无回测':'该范围暂无任务记录'}}</p><div class="table-wrap"><Table><TableHeader><TableRow><TableHead>任务</TableHead><TableHead v-if="tab==='backtests'">策略版本</TableHead><TableHead>状态</TableHead><TableHead>创建时间</TableHead><TableHead>结果</TableHead></TableRow></TableHeader><TableBody><TableRow v-for="t in displayedTasks" :key="t.id"><TableCell>{{t.name||'任务记录'}}</TableCell><TableCell v-if="tab==='backtests'"><Button variant="ghost" size="sm" :disabled="!versions.some(v=>v.id===t.strategyVersionId)" @click="versionSelection=t.strategyVersionId">{{versionLabel(t.strategyVersionId)}}</Button></TableCell><TableCell><QuantStatusBadge :status="t.status" /></TableCell><TableCell>{{formatTime(t.createdAt)}}</TableCell><TableCell><Button size="sm" variant="outline" as-child><RouterLink :to="{path:'/quant/tasks',query:{type:t.resource,id:t.id}}">查看进度 / 报告</RouterLink></Button></TableCell></TableRow></TableBody></Table></div></div></template>
 <Dialog v-model:open="historyOpen"><DialogContent class="qw max-h-[88vh] overflow-y-auto sm:max-w-[960px]" :aria-describedby="undefined"><DialogHeader><DialogTitle>历史版本</DialogTitle></DialogHeader><div class="px-1 pb-1"><Table><TableHeader><TableRow><TableHead>版本</TableHead><TableHead>保存时间</TableHead><TableHead>操作</TableHead></TableRow></TableHeader><TableBody><TableRow v-for="v in versions" :key="v.id"><TableCell>v{{v.version}}</TableCell><TableCell>{{formatTime(v.createdAt)}}</TableCell><TableCell><Button variant="ghost" @click="versionSelection=v.id;tab='backtests';historyOpen=false">查看回测记录</Button></TableCell></TableRow></TableBody></Table></div></DialogContent></Dialog>
