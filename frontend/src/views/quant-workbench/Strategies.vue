...
const universeName = id => universes.value.find(u => String(u.id) === String(id))?.name || id || '未配置'
...
<template>
  <QuantPageHeader title="策略管理"><template #actions><Button @click="router.push('/quant/strategies/new')">创建策略</Button></template></QuantPageHeader>
  ...
  <Dialog :open="!!deleting" @update:open="value=>{if(!value&&!busy)deleting=null}">
    <DialogContent>
      ...
      <DialogFooter><Button variant="outline" :disabled="busy" @click="deleting=null">取消</Button><Button variant="destructive" :disabled="busy" @click="removeStrategy">{{busy?'正在删除…':'确认删除'}}</Button></DialogFooter>
    </DialogContent>
  </Dialog>
</template>
