...
async function load(){[rows.value,assets.value]=await Promise.all([quant.list('universes'),quant.list('assets')])}
function edit(row){form.value=row?clone(row):{name:'',assetClass:'STOCK',assetIds:[]}}
function save(){run(async()=>{if(form.value.id)await quant.update('universes',form.value.id,form.value);else await quant.create('universes',form.value);form.value=null;await load()})}
function act(row,action){run(async()=>{await quant.action('universes',row.id,action);await load()})}
onMounted(()=>run(load))
</script>
<template>
 <QuantPageHeader title="资产池"><template #actions><Button @click="edit()">创建资产池</Button></template></QuantPageHeader>
 <Dialog :open="!!form" @update:open="value=>{if(!value)form=null}"><DialogContent class="qw max-h-[88vh] overflow-y-auto sm:max-w-[960px]" :aria-describedby="undefined"><DialogHeader><DialogTitle>{{form?.id?'编辑资产池':'创建资产池'}}</DialogTitle></DialogHeader><p v-if="error" class="error" role="alert">{{error}}</p><form v-if="form" class="space-y-5 px-1 pb-1" @submit.prevent="save"><div class="fields"><label class="field">名称<Input v-model.trim="form.name" required maxlength="120" /></label><label class="field">资产类别<select v-model="form.assetClass" @change="form.assetIds=[]"><option value="STOCK">股票</option><option value="ETF">ETF</option><option value="FUND">场外基金</option></select></label></div>
 ...
 <div class="toolbar mt-4"><Button type="submit" :disabled="busy||!form.assetIds.length||form.status==='ARCHIVED'">保存资产池</Button><Button type="button" variant="outline" @click="form=null">关闭</Button><span class="muted">已选 {{form.assetIds.length}} 个</span></div></form></DialogContent></Dialog>
