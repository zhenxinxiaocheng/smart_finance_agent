...
public class WorkbenchService {
    ...
    Map<String,Object> view(Map<String,Object> r) {
        ...
        if("strategies".equals(r.get("kind"))) {
            var latest=db.queryForList("SELECT * FROM quant_v2_task WHERE user_id=? AND strategy_id=? AND kind='backtests' AND status<>'DELETED' ORDER BY created_at DESC LIMIT 1",r.get("user_id"),r.get("id"));
    ...
    public List<Map<String,Object>> list(Long u,String kind) {
        ...
        return ("deployments".equals(kind)?db.queryForList("SELECT * FROM "+table+" WHERE user_id=? AND status<>'DELETED' ORDER BY created_at DESC",u):db.queryForList("SELECT * FROM "+table+" WHERE user_id=? AND kind=? AND status<>'DELETED' ORDER BY created_at DESC",u,kind)).stream().map(this::view).toList();
    }
    ...
    public Map<String,Object> cancel(Long u,String kind,String identity) {get(u,kind,identity);db.update("UPDATE quant_v2_task SET status='CANCELLED',stage='CANCELLED',claim_token=NULL,lease_until=NULL,updated_at=? WHERE id=? AND user_id=? AND status IN ('QUEUED','RUNNING')",now(),identity,u);return get(u,kind,identity);}
    public Map<String,Object> retry(Long u,String kind,String identity) { return tx.execute(txStatus->{
        get(u,kind,identity);var old=row("quant_v2_task",u,identity);
