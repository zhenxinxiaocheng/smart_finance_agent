...
DROP TABLE IF EXISTS `quant_universe_member`;

DROP TABLE IF EXISTS `quant_universe_snapshot`;
...
CREATE TABLE `investment_product` (
    ...
    `exchange_code` VARCHAR(30) NULL,
    `listing_date` DATE NULL,
    `delisting_date` DATE NULL,
    `source_metadata` VARCHAR(500) NULL,
    PRIMARY KEY (id)
);
...
CREATE UNIQUE INDEX market_data_job_uk_market_data_job ON `market_data_job` (product_id, job_type);
CREATE INDEX market_data_job_idx_queue ON `market_data_job` (status, next_retry_at, updated_at);
...
CREATE UNIQUE INDEX quant_universe_snapshot_uk_revision ON `quant_universe_snapshot` (universe_id, revision);
CREATE INDEX quant_universe_snapshot_idx_asof ON `quant_universe_snapshot` (user_id, universe_id, as_of_date);
